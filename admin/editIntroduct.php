...
<div class="grid_12">
	...
	<div class="module">
		 <h2><span>Sửa thông tin hoa</span></h2>
			
		 <div class="module-body">
			<?php
				if(isset($_GET["edit"])){
					$EDIT=$_GET["edit"];
					$sqle="SELECT * FROM news WHERE id_news='{$EDIT}'";
				...
				if(isset($_POST['sua'])){
					...
					if($dem>0){
						...
						if($tenhinh == NULL){
							$query = "UPDATE hoa SET Tenhoa='$tenhoa', Giaban='$giaban', Hinhanh='', Mota='$mota', id_theloai='$dmtheloai', id_nhom='$dmnhom' WHERE id_hoa=$EDIT LIMIT 1
								";
							if($result_query){
									header("location:indexIntroduct.php?msg=Bạn đã thêm thành công!");
									exit();
								}else{
							...
							if($ketqua){
								$query = "UPDATE hoa SET Tenhoa='$tenhoa', Giaban='$giaban', Hinhanh='$tenhinh', Mota='$mota', id_theloai='$dmtheloai', id_nhom='$dmnhom' WHERE id_hoa=$EDIT LIMIT 1
								";
